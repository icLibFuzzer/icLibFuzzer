...
class TracePC {
 public:
  ...
  void Initialize() {
    ...
    int Res = snprintf(FuzzingName, 0xff, "/dev/shm/libfuzzer_%d", FuzzingNumber);
    assert(Res > 0 && "create share file name");
    int fd = open(FuzzingName, O_CREAT | O_RDWR, 0600);
...
private:
  ...
  struct { struct PCTableEntry *Start, *Stop; } ModulePCTable[4096];
  size_t NumPCTables;
