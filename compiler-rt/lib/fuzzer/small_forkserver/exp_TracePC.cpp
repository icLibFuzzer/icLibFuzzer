...
void TracePC::HandleInline8bitCountersInit(uint8_t *Start, uint8_t *Stop) {
  ...
  size_t R = 0;
  if (NeedFirst)
    M.Regions[R++] = {Start, std::min(Stop, AlignedStart)};
  for (uint8_t *P = AlignedStart; P < AlignedStop; P += PageSize())
    M.Regions[R++] = {P, P + PageSize()};
  if (NeedLast)
    M.Regions[R++] = {AlignedStop, Stop};
...
void TracePC::AddValueForMemcmp(void *caller_pc, const void *s1, const void *s2,
  ...
  bool Ret = TORCW.Insert(Idx ^ Hash, B1, B2, Len);
    if (Ret) 
        RecordOverwrite("TORCW");
}
...
void TracePC::HandleCmp(uintptr_t PC, T Arg1, T Arg2) {
  ...
  if (sizeof(T) == 4) {
      bool Ret = TORC4.Insert(ArgXor, Arg1, Arg2);  
    if (Ret) 
        RecordOverwrite("TORC4");
  }
  else if (sizeof(T) == 8) {
      bool Ret = TORC8.Insert(ArgXor, Arg1, Arg2);  
    if (Ret) 
        RecordOverwrite("TORC8");
  }
...
void __sanitizer_weak_hook_strstr(void *called_pc, const char *s1,
  ...
  bool Ret = TPC.MMT.Add(reinterpret_cast<const uint8_t *>(s2), strlen(s2));
  if (Ret) {
      TPC.RecordOverwrite("MMT");
  }
}

ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY
void __sanitiizer_weak_hook_strcasestr(void *called_pc, const char *s1,
                                      const char *s2, char *result) {
    bool Ret = TPC.MMT.Add(reinterpret_cast<const uint8_t *>(s2), strlen(s2));
  if (Ret) {
      TPC.RecordOverwrite("MMT");
...
void __sanitizer_weak_hook_memmem(void *called_pc, const void *s1, size_t len1,
    ...
    bool Ret = TPC.MMT.Add(reinterpret_cast<const uint8_t *>(s2), len2);
  if (Ret) {
      TPC.RecordOverwrite("MMT");
  }
